...
raspberry_pi_ip = "192.168.1.10"  # Replace with the Raspberry Pi's IP address
    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            ...
            if stage == 'calibration_open':
                ...
                servo_angles = [openness_to_servo_angle(openness_percentage[i]) for i in range(5)]
                ...
                # Send servo angles to Raspberry Pi
                data = ','.join([str(int(angle)) for angle in servo_angles])
                sock.sendto(data.encode(), (raspberry_pi_ip, raspberry_pi_port))
